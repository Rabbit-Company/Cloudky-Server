...
export default async function handleAccountToken(req: Request, match: MatchedRoute | null, ip: string | undefined): Promise<Response> {
	...
	if(auth === null) return Utils.jsonError(1011);

	if(!Validate.username(auth.user)) return Utils.jsonError(1012);
	if(!Validate.password(auth.pass)) return Utils.jsonError(1013);

	let result = await DB.prepare('SELECT "Password" FROM "Accounts" WHERE "Username" = ?', [auth.user]);
	if(result === null) return Utils.jsonError(2000);
	if(result.length !== 1) return Utils.jsonError(1012);

	if(!(await Bun.password.verify(auth.pass, result[0].Password))) return Utils.jsonError(1014);
	...
	if(token === null) return Utils.jsonError(1015);
