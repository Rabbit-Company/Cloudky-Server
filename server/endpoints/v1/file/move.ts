...
export default async function handleFileMove(req: Request, match: MatchedRoute | null, ip: string | undefined): Promise<Response> {
	...
	if(!Validate.userFilePathNames(data.files)) return Utils.jsonResponse(Errors.getJson(1005));
	if(!Validate.userFilePathNames([data.destination])) return Utils.jsonResponse(Errors.getJson(1005));
